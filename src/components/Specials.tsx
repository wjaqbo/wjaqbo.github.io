...
export default function Specials({ ...props }) {
  return (
    <section
      className={`items-center justify-center overflow-hidden bg-green-700 p-20 text-white md:flex ${props.className}`}
    >
      <div className="flex basis-2/4 flex-col items-center justify-center">
        <h2
          data-aos={"fade-right"}
          className="mb-10 text-center text-4xl font-bold leading-relaxed md:mb-0"
        >
          Today Specials
        </h2>
        {/* <Image
      ...
      <ul className="mt-10 flex flex-col gap-6 md:mt-0">
        {specials.map((item) => (
          <li
            data-aos={"fade-left"}
            key={item.id}
            className="border-b border-emerald-400 pb-6 opacity-50 last:border-none"
          >
